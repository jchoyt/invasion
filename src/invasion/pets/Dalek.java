...
public class Dalek
  extends Critter
{
  public static final String KEY = Dalek.class.getName();
  ...
  public Dalek(int station)
  {
    ...
    init();
    insert();
    log.finer("Finished creating Dalek " + this.id + " at location " + b.getLocation());
  }
  ...
  public void init()
  {
    ...
    this.shield = 120;
    this.shieldMax = 120;
    this.name = ("Dalek " + (int)(Math.random() * 2500.0D));
    this.typeid = 1;
    this.damageType = 'e';
    if (Math.random() < 0.01D) this.name = "I blame Player_1";
    this.deathKnells = new String[] { "You have landed the killing blow.", "With a final blow, the lights on the Dalek slowly dim.", "Sparks fly from your target's caraprice and it finally lies motionless.", "You yell, \"Holy crap! Did you see that shot?!??  Did you?\"  Oh. Yeah. There's nobody else here. You are forever alone." };
  }
