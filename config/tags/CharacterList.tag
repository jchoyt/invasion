<%@ tag import="java.util.*, invasion.dataobjects.*, invasion.util.*, java.sql.*" %><table border="0" cellpadding="2" cellspacing="0" width="700px">
        <thead>
            <tr><th>Name</th><th>Level</th><th>AP</th><th>IP</th><th>HP</th><th>Station</th><th>Location</th><th>&nbsp;</th></tr>
        </thead>
        <tbody>
<%
    String player = request.getRemoteUser();

    String query = "select a.id, a.name as alt, a.level, ap || '/' || apmax as apset, ip, hp || '/' || hpmax as hpset, s.name as sname, 'L' || l.level || ' (' || x || ',' || y || ')' as loc from alt a join location l on (a.location=l.id) join station s on (s.id=l.station) where username=? order by a.name";
    ...
    while(rs.next())
    {
        ...
        {
            out.write( "</td><td colspan=\"2\">Dead");
